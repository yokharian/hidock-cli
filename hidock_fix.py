...
class HiDockToolGUI:
    ...
    def _update_menu_states(self): # Now also updates toolbar button states
        ...
        if hasattr(self, 'toolbar_download_button'):
            self.toolbar_download_button.config(state=tk.NORMAL if is_connected and has_selection and not self.is_long_operation_active else tk.DISABLED)

        if hasattr(self, 'toolbar_play_button'):
             self.toolbar_play_button.config(state=tk.NORMAL if can_play_selected and not self.is_long_operation_active else tk.DISABLED)

        if hasattr(self, 'toolbar_delete_button'):
            self.toolbar_delete_button.config(state=tk.NORMAL if is_connected and has_selection and not self.is_long_operation_active else tk.DISABLED)
