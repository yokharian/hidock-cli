...
class AudioProcessor:
        ...
        try:
            ...
            if PYDUB_AVAILABLE:
                try:
                    ...
                    logger.warning("AudioProcessor", "get_audio_info", f"Pydub failed for {filepath}: {e}")
            ...
            if info['format'] == '.wav' and info['duration'] == 0.0:
                try:
                    ...
                    logger.warning("AudioProcessor", "get_audio_info", f"Wave module failed for {filepath}: {e}")
            
            return info
            
        except Exception as e:
            logger.error("AudioProcessor", "get_audio_info", f"Error getting audio info for {filepath}: {e}")
            return {}
    ...
    def convert_audio_format(input_path: str, output_path: str, target_format: str = "wav") -> bool:
        ...
        if not PYDUB_AVAILABLE:
            logger.error("AudioProcessor", "convert_audio_format", "Pydub not available for conversion")
            return False
        
        try:
            audio = AudioSegment.from_file(input_path)
            audio.export(output_path, format=target_format)
            logger.info("AudioProcessor", "convert_audio_format", f"Converted {input_path} to {output_path}")
            return True
        except Exception as e:
    ...
    def normalize_audio(input_path: str, output_path: str, target_dBFS: float = -20.0) -> bool:
        ...
        if not PYDUB_AVAILABLE:
            logger.error("AudioProcessor", "normalize_audio", "Pydub not available for normalization")
            return False
        ...
        try:
            ...
            logger.info("AudioProcessor", "normalize_audio", f"Normalized {input_path} to {target_dBFS} dBFS")
            return True
        except Exception as e:
    ...
    def extract_waveform_data(filepath: str, max_points: int = 1000) -> Tuple[np.ndarray, int]:
        ...
        try:
            if filepath.lower().endswith('.wav'):
                ...
                logger.error("AudioProcessor", "extract_waveform_data", "No audio processing library available")
                return np.array([]), 0
                
        except Exception as e:
            logger.error("AudioProcessor", "extract_waveform_data", f"Error extracting waveform: {e}")
            return np.array([]), 0
...
class AudioPlaylist:
    ...
    def remove_track(self, index: int) -> bool:
        ...
        try:
            ...
            logger.error("AudioPlaylist", "remove_track", f"Error removing track at index {index}: {e}")
            return False
...
class EnhancedAudioPlayer:
    ...
    def _initialize_audio_backend(self):
        ...
        if not PYGAME_AVAILABLE:
            logger.error("EnhancedAudioPlayer", "_initialize_audio_backend", "Pygame not available")
            return
        
        try:
            if not pygame.mixer.get_init():
                pygame.mixer.init(frequency=44100, size=-16, channels=2, buffer=1024)
            logger.info("EnhancedAudioPlayer", "_initialize_audio_backend", "Audio backend initialized")
        except Exception as e:
            logger.error("EnhancedAudioPlayer", "_initialize_audio_backend", f"Failed to initialize audio: {e}")
    ...
    def load_track(self, filepath: str) -> bool:
        ...
        try:
            ...
            logger.error("EnhancedAudioPlayer", "load_track", f"Error loading track {filepath}: {e}")
            return False
    ...
    def play(self) -> bool:
        ...
        try:
            ...
            if self.state == PlaybackState.PAUSED:
                ...
                pygame.mixer.music.load(current_track.filepath)
    ...
    def seek(self, position: float) -> bool:
        ...
        try:
            ...
            logger.error("EnhancedAudioPlayer", "seek", f"Error seeking to position {position}: {e}")
            return False
    ...
    def next_track(self) -> bool:
        ...
        try:
            ...
            logger.error("EnhancedAudioPlayer", "next_track", f"Error skipping to next track: {e}")
            return False
    ...
    def previous_track(self) -> bool:
        ...
        try:
            ...
            logger.error("EnhancedAudioPlayer", "previous_track", f"Error skipping to previous track: {e}")
            return False
            ...
            try:
                if self.state == PlaybackState.PLAYING and PYGAME_AVAILABLE:
                    if pygame.mixer.music.get_busy():
                        ...
                        current_track = self.playlist.get_current_track()
                        if current_track and self.current_position >= current_track.duration:
                ...
                logger.error("EnhancedAudioPlayer", "_position_update_worker", f"Error in position thread: {e}")
                break
    ...
    def _notify_position_changed(self):
        ...
        if self.on_position_changed:
            try:
                ...
                logger.error("EnhancedAudioPlayer", "_notify_position_changed", f"Error in position callback: {e}")
    ...
    def _notify_state_changed(self):
        ...
        if self.on_state_changed:
            try:
                ...
                logger.error("EnhancedAudioPlayer", "_notify_state_changed", f"Error in state callback: {e}")
    ...
    def _notify_track_changed(self):
        ...
        if self.on_track_changed:
            try:
                ...
                logger.error("EnhancedAudioPlayer", "_notify_track_changed", f"Error in track callback: {e}")
    ...
    def _notify_playlist_changed(self):
        ...
        if self.on_playlist_changed:
            try:
                ...
                logger.error("EnhancedAudioPlayer", "_notify_playlist_changed", f"Error in playlist callback: {e}")
