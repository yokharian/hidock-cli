...
class HiDockJensen:
        ...
        with self._usb_lock:  # Ensure connect/disconnect are serialized
            ...
            try:
                ...
                if self.device is None:
                    ...
                    logger.warning(
                        "Jensen", "connect", f"Failed to connect: {error_msg}"
                    )
                ...
                try:
                    ...
                    if e_cfg.errno == 16:  # Resource busy
                        ...
                        logger.error(
                            "Jensen", "connect", f"Unexpected USBError during set_configuration: {e_cfg}"
                        )
                ...
                if self.device.idProduct == 0xAF0C:
                    self.model = "hidock-h1"
                elif self.device.idProduct == 0xAF0D:
                    self.model = "hidock-h1e"
                elif self.device.idProduct == 0xAF0E:
                    self.model = "hidock-p1"
                elif self.device.idProduct == pid:
                    self.model = f"HiDock Device (PID: {hex(pid)})"
                else:
                    self.model = f"unknown (PID: {hex(self.device.idProduct)})"
                ...
                if isinstance(e, usb.core.USBError):
                    if e.errno == 13:  # Access Denied
                        specific_error_message = "Access denied to device. It might be in use "\
                ...
                logger.error("Jensen", "connect", f"Connection failed: {e}\n{traceback.format_exc()}")
                # Log the user-friendly message that will be returned
                logger.info("Jensen", "connect", f"Returning failure with message: {specific_error_message}")
                self.disconnect()  # Ensure cleanup on failed connection
                return False, specific_error_message
            except (OSError, RuntimeError) as e:  # Fallback for other known OS/Runtime errors
        ...
        with self._usb_lock:
            ...
            if self.device:
                try:
                    ...
                    logger.warning("Jensen", "disconnect", f"Error during interface release/reattach: {e}")
    ...
    def _send_command(self, command_id, body_bytes=b"", timeout_ms=5000):
        """
        Sends a command packet to the device's OUT endpoint.
        ...
        try:
            ...
            if e.errno == 32:  # LIBUSB_ERROR_PIPE (stall)
                try:
                    ...
                    logger.info("Jensen", "_send_command", "Cleared halt on EP_OUT after pipe error.")
                except usb.core.USBError as ce:  # More specific
                    logger.error("Jensen", "_send_command", f"Failed to clear halt on EP_OUT: {ce}")
    ...
    def _receive_response(self, expected_seq_id, timeout_ms=5000, streaming_cmd_id=None):
        ...
        while time.time() - start_time < overall_timeout_sec:
            try:
                ...
                pass  # Expected if no data is ready on the endpoint
            except usb.core.USBError as e:
                ...
                if e.errno == 32:  # LIBUSB_ERROR_PIPE
                    try:
                        ...
                        logger.info("Jensen", "_receive_response", "Cleared halt on EP_IN after pipe error.")
                    except usb.core.USBError as ce:  # More specific
    ...
    def _send_and_receive(self, command_id, body_bytes=b"", timeout_ms=5000):
        ...
        with self._usb_lock:  # Ensure send and receive are atomic relative to other commands
            ...
            except (usb.core.USBError, ConnectionError) as e:  # Catch errors from send or receive
    ...
    def get_device_info(self, timeout_s=5):
        ...
        if response and response["id"] == CMD_GET_DEVICE_INFO:
            ...
            if len(body) >= 4:
                ...
                logger.error("Jensen", "get_device_info", "Response body too short for version information.")
        else:
    ...
    def list_files(self, timeout_s=20):
        ...
        Returns:
            dict or None: A dictionary containing 
                {"files": list_of_file_details, "totalFiles": count, "totalSize": bytes}
                          if successful, None or a dict with an "error" key otherwise.
        """
        ...
        while offset < len(data_view):
            try:
                ...
                offset += 1
                name_len = struct.unpack(">I", b"\x00" + data_view[offset: offset + 3])[0]
                offset += 3
                if offset + name_len > len(data_view):
                    break
                filename = "".join(chr(b) for b in data_view[offset: offset + name_len] if b > 0)
                offset += name_len
                ...
                if file_version == 1:
                    duration_sec = (file_length_bytes / 32) * 2
                elif file_version == 2:
                    duration_sec = (file_length_bytes - 44) / (48000 * 2 * 1) if file_length_bytes > 44 else 0
                elif file_version == 3:
                    duration_sec = (file_length_bytes - 44) / (24000 * 2 * 1) if file_length_bytes > 44 else 0
                elif file_version == 5:
                    duration_sec = file_length_bytes / 12000
                else:
                    duration_sec = file_length_bytes / (16000 * 2 * 1)
                files.append(
                    {
                        "name": filename,
                ...
                if (total_files_from_header != -1 and parsed_file_count >= total_files_from_header) or (
                    expected_files_from_count_cmd != -1 and parsed_file_count >= expected_files_from_count_cmd
                ):
                    break
        ...
        with self._usb_lock:
            ...
            try:
                ...
                initial_seq_id = self._send_command(
                    CMD_TRANSFER_FILE, filename.encode("ascii", errors="ignore"), timeout_ms=10000
                )
            ...
            # more specific in this context due to OSError's broadness) should come first.
            except (usb.core.USBError, ConnectionError) as e_usb_conn:  # Specific to USB/connection
                logger.error(
                    "Jensen",
                    "stream_file",
            ...
            except (IOError, OSError) as e_io_os:  # Specific to file operations and other OS errors
                ...
                # Clear receive buffer after streaming, regardless of outcome
                self.receive_buffer.clear()
                if status_to_return not in ["OK", "cancelled"] and self.device and self.ep_in:
                    logger.debug(
                        "Jensen",
                        "stream_file",
                    ...
                    for _ in range(20):
                        try:
                            if not self.device.read(self.ep_in.bEndpointAddress, self.ep_in.wMaxPacketSize, timeout=50):
                                break
                        except usb.core.USBTimeoutError:
                            break
    ...
    def delete_file(self, filename, timeout_s=10):
        ...
        return {"result": "failed", "code": -1, "error": "No or invalid response from device"}
    ...
    def get_card_info(self, timeout_s=5):
        ...
        if response and response["id"] == CMD_GET_CARD_INFO and len(response["body"]) >= 12:
            ...
            return {"used": used_mb, "capacity": capacity_mb, "status_raw": status_raw}
    ...
    def format_card(self, timeout_s=60):
        ...
        """
        response = self._send_and_receive(
            CMD_FORMAT_CARD, body_bytes=bytes([1, 2, 3, 4]), timeout_ms=int(timeout_s * 1000)
        )
        if response and response["id"] == CMD_FORMAT_CARD:
            ...
            return {"result": status_str, "code": result_code}
        logger.error("Jensen", "format_card", f"Failed format card response. Response: {response}")
        return {"result": "failed", "code": -1, "error": "No or invalid response from device"}
    ...
    def get_device_time(self, timeout_s=5):
        ...
        if response and response["id"] == CMD_GET_DEVICE_TIME:
            if response["body"]:
                ...
                logger.error("Jensen", "get_device_time", "Response body for time is missing or empty.")
                return {"time": "unknown", "error": "Empty response body"}
        logger.error("Jensen", "get_device_time", "Failed to get device time or invalid response ID.")
        return None
    ...
    def get_file_block(
        ...
        with self._usb_lock:
            ...
            try:
                ...
                body = struct.pack(">I", block_length) + filename.encode("ascii", errors="ignore")
                initial_seq_id = self._send_command(CMD_GET_FILE_BLOCK, body, timeout_ms=10000)
                ...
                while bytes_received < block_length:
                    ...
                    response = self._receive_response(initial_seq_id, 15000, streaming_cmd_id=CMD_GET_FILE_BLOCK)
                ...
                status_to_return = "fail_disconnected" if not self.is_connected() else "fail_exception"
            finally:
